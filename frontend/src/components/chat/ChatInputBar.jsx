...
const ChatInputBar = ({ input, onInputChange, onKeyDown, onSend, onEnd, onNext, isActive }) => (
  ...
  >
    <div className="flex items-center gap-2">
      ...
      <button
        onClick={onNext}
        className="flex items-center gap-1.5 bg-white/[0.04] border border-white/[0.08] text-[#9CA3AF] rounded-xl px-3.5 py-2.5 text-xs font-semibold cursor-pointer shrink-0 transition-all duration-200 hover:bg-emerald-500/10 hover:border-emerald-500/30 hover:text-emerald-400"
      >
        Next
        <svg fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24" className="w-3.5 h-3.5">
